...
s = spinw;
s.genlattice('lat_const',[6 6 5],'angled',[90 90 120], 'spgr', 147)
s.addatom('r', [1/2 0 0],'S', 1,'label','MCu1','color','blue')
plot(s,'range',[2 2 1],'atomColor','gold')
...
s.gencoupling('maxDistance', 4)
disp('Bonds (length in Angstrom):')
s.table('bond')

%% Add J1 using addmatrix and addcoupling.
s.addmatrix('value',-1,'label','J1','color','green')
s.addcoupling('mat','J1','bond', 1);

%% Plot a nice rectangular cutout of the kagome lattice
plot(s,'range',[31 32 2],'unit','xyz','cellMode','single')
...
s.genmagstr('mode','helical','k',[0 0 0],'n',[0 1 0],'S',[0 1 0]')
...
spec = s.spinwave({[-1/2 0 0] [0 0 0] [1/2 1/2 0] 100},'hermit',false);
fmkSpec = sw_neutron(spec);
% Which component is observed in neutron scattering? 
...
%% Calulate the powder spectra from 0-2.5A and 0-7 meV. 
% Use the help of @spinw/powspec or ask for the correct terminology. 
% How does the number of samples effect the calculation?

fmkPow = s.powspec(linspace(0,2.5,100),'Evect',linspace(0,7,250),'nRand',1000,'hermit',false);
figure
subplot(2,1,1)
sw_plotspec(fmkPow,'colorbar',true,'axLim',[0 0.05])
subplot(2,1,2)
% Look at the documentation on how to normalize a  sepectra
sw_plotspec(fmkPow,'colorbar',true,'axLim',[0 0.05], 'norm', true)
...
nQ = 1E3; % Or however many points can be calculated in a sensible amount of time.
Q = linspace(0, 1, nQ);

specDOS = s.powspec(Q,'Evect',linspace(0,7,250), 'nRand', 1, 'component', 'Sxx+Syy+Szz');
specDOS = sw_instrument(specDOS, 'dE', 0.1);
I = sum(specDOS.swConv, 2)/nQ;
